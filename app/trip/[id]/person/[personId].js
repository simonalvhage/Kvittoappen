...
import {
  getTrip, getReceiptsByTripAndPerson, listPersons,
} from '../../../../src/lib/db';
import { formatSEK } from '../../../../src/lib/fx';
import { EmptyState } from '../../../../src/components/EmptyState';

export default function TripPersonDetail() {
  const { c } = useTheme();
  ...
  const load = useCallback(async () => {
    const [t, rs, persons] = await Promise.all([
      getTrip(id),
      getReceiptsByTripAndPerson(id, personId),
      listPersons(),
    ]);
    setTrip(t);
    setReceipts(rs);
    setPerson(persons.find((x) => String(x.id) === String(personId)) || null);
    setLoading(false);
  }, [id, personId]);
  ...
  if (!person || !trip) {
    return (
      <View style={[styles.center, { backgroundColor: c.bg }]}>
        <Text style={{ color: c.textSecondary }}>Kunde inte hitta personen.</Text>
      </View>
    );
  }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.bg }}>
      <Stack.Screen options={{ title: person.name, headerBackTitle: 'Tillbaka' }} />
      <ScrollView contentContainerStyle={styles.content}>
        <View style={[styles.hero, { backgroundColor: c.card }, shadow.card]}>
          ...
          <Text style={[font.title2, { color: c.text, marginTop: spacing.sm }]}>{person.name}</Text>
        ...
        {receipts.length === 0 ? (
          <View style={{ marginTop: spacing.xl }}>
            <EmptyState
              icon="people-outline"
              title="Inga kvitton kopplade till denna person"
              message="Tilldela personer i kvittodetaljen för att se dem här."
            />
          </View>
        ) : null}
